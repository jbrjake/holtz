...
- [ ] Running `python -m pytest --cov=skills/holtz/scripts --cov=hooks --cov-report=term-missing` and checking the badge value match

**Validation Command:**
```bash
python -m pytest --cov=skills/holtz/scripts --cov=hooks --cov-report=term-missing --cov-fail-under=60 --tb=no -q 2>&1 | grep "Total coverage" | grep -oP '\d+' | head -1
```
...
event = {'tool_input': {'command': 'ruff check .'}, 'cwd': '/tmp/fake-cwd'}
# Would need a cache with stall > 15 to test the block path
print('Manual test: create enforcement cache with stall=16, run commit_gate with ruff command')
"
```
...
result = _check_bash_write('bash -c \"python3 -c \\\"import os; os.remove(\\\\\\\"enforcement/hooks/test.py\\\\\\\")\\\"\"')
print('Blocked:' if result else 'Bypassed (expected)')
"
```
