...
- **Subagents for heavy work — scanning AND fixes.** Delegate grep/read-heavy scanning (test-file audits, module scans) *and* per-finding fix investigation to Agent subagents. The code they read and the hypotheses they test stay in THEIR context, not yours. A scan subagent returns a short summary + writes findings to disk; a fix subagent investigates one finding and returns a failing test + a minimal fix diff + evidence, which the **orchestrator** (you) applies, verifies, commits, and records via Sahjhan. Never let a fix subagent commit or drive protocol state — that stays linear in the orchestrator. See [references/phase-fix-loop.md](references/phase-fix-loop.md).
